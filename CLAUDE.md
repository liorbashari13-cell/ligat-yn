...
Hebrew **RTL** landing page for a youth football (soccer) league in Netanya, Israel — **ליגת יראנו ניסים** (Yaranu Nisim League). Single-page marketing site, animation-heavy, with a multi-step team-registration form backed by Google Apps Script + Sheets.
...
- **Apps Script POST quirk:** `src/lib/api.js` submits the registration with `mode: 'no-cors'` + `text/plain`. Apps Script web apps don't expose CORS headers, so a normal fetch that reads the response is blocked ("Failed to fetch"). `no-cors` lets the POST through (the row is written) but returns an **opaque** response we can't read — so `submitRegistration` resolves optimistically and the "full" state comes from the GET counter, which the form refreshes after submit. `text/plain` keeps it a simple request (no preflight); `e.postData.contents` is still the raw JSON string server-side.
...
Player portraits in `src/sections/Players.jsx` are styled placeholders (football icon + name); swap in real images where marked `// Replace placeholder with real image when received`. The logo is `public/logo.png`, referenced as `/logo.png`.
