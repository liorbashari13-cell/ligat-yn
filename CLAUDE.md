...
- **Apps Script POST quirk:** `src/lib/api.js` sends the registration body as `text/plain` (not `application/json`) on purpose — Apps Script web apps don't answer the CORS preflight a JSON content-type triggers; `e.postData.contents` is still the raw string, so `JSON.parse` works server-side.
